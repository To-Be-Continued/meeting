...
create table meeting_t  (
    ...
    m_3DSign          boolean not null,                               /*会议是否开启签名*/
    m_num             int not null,                                   /*会议参与人数*/
    m_pwd             char(32) Default 'e10adc3949ba59abbe56e057f20f883e'                           /*密码，将输入得密码用MD5加密后存储,增加字段*/
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table meeting_sign(
    s_id             bigint auto_increment not null primary key,     /*签名实践ID*/
    m_id             bigint not null,                                /*会议ID*/
    u_id             bigint not null,                                /*发起人ID*/
    s_imgpath        varchar(200),                                   /*签名墙图片*/
    s_starttime      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,            /*开始时间*/
    s_endtime        TIMESTAMP not null,                             /*结束时间*/
    foreign key(m_id)references meeting_t(m_id) on delete cascade on update cascade,
    foreign key(u_id)references user_t(u_id) on delete cascade on update cascade
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table sign_img(
    si_id            bigint auto_increment not null primary key,     /*签名图片ID*/
    s_id             bigint not null,                                /*签名事件ID*/
    u_id             bigint not null,                                /*用户ID*/
    si_imgpath       varchar(200),                                   /*签名图片*/
    foreign key(s_id)references meeting_sign(s_id) on delete cascade on update cascade,
    foreign key(u_id)references user_t(u_id) on delete cascade on update cascade
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
